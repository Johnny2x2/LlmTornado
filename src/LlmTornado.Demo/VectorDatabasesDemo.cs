...
{
    [TornadoTest]
    [Flaky("Requires Chroma server running")]
    ...
    [TornadoTest]
    [Flaky("Requires Milvus server running")]
    public static async Task TestMilvus()
    {
        string query = "Function to add two numbers in python";
        string embeddingDescription = "A function that adds two numbers together in python";
        ...
        MilvusVectorDatabase vectorDb = new MilvusVectorDatabase(
            host: "localhost",
            port: 19530,
            vectorDimension: 1536
        );

        await vectorDb.InitializeCollection("functions");
        ...
        await vectorDb.AddDocumentsAsync([vectorDocument]);

        //Query DB for relevant functions
        var queryData = await vectorDb.QueryByEmbeddingAsync(dataQuery, topK: 5);
